...
- [x] **Privacy leakage through over-parameterization.** The spec explicitly warns that a
...
- [x] **Tools as attack targets.** The spec notes that tools wrapping sensitive/high-
...
- [x] **`annotations.readOnlyHint` is a hint, not a boundary.** The spec is explicit that
...
- [x] **Same-origin boundary risk in multi-origin agent browsing** is called out as an
...
- [x] **BFCache / document lifecycle.** Per spec, a document's registered tools are
...
- [x] **Never let a tool call be the final, irreversible action.** Submission, payment,
...
- [x] **Re-validate every input server-side** with Pydantic, even though the `inputSchema`
...
- [x] **Use the uncommitted/proposal pattern** (see `ARCHITECTURE.md`) for any tool that
      writes data. Committing requires a separate, human-only endpoint.

## 2. Prompt injection via content the agent reads

- [x] Text extracted from uploaded documents (`extract_from_document`) must be treated as
...
- [x] Same applies to any field where a human can type free text that later gets shown
      back to an agent (e.g. a "notes" field) — sanitize before it's surfaced in a tool
      response.

## 3. Tool design hygiene

- [x] No "god tools." A tool like `update_anything(field, value)` that accepts an arbitrary
      field name is a privilege-escalation risk — enumerate allowed fields explicitly in
      the schema and re-check server-side.
- [x] Tool descriptions are honest and minimal. Don't write a description implying a tool
...
- [x] Idempotency: tools an agent might retry (e.g. after a timeout) shouldn't double-write.
      Use the session + field_name as an upsert key, not an insert-only log for `FieldValue`.

## 4. Standard web security, still required

- [x] CSRF protection on every mutating `/api/*` endpoint — WebMCP tools call these via
      `fetch()` from the same origin, so standard same-origin + CSRF-token checks apply.
- [x] Session cookies: `HttpOnly`, `Secure`, `SameSite=Lax` at minimum.
- [x] File upload validation: enforce MIME type allowlist, max size, and never execute or
...
- [x] Output encoding: any agent-proposed or document-extracted text rendered into
      `review.html` goes through Jinja2's autoescaping — never build raw HTML strings
      from tool output.
- [x] Rate limiting on tool-backing endpoints (see `HOOKS.md` pre-execute hook) — an agent
...
- [x] No secrets in `webmcp-tools.js` or any static file — API keys, DB credentials, etc.
      live only in the Python backend's environment variables.

## 5. Origin and exposure scope

- [x] Do not set `exposedTo` on any tool unless you have a specific, documented reason to
...
- [x] If you ever add an embedded/author-provided agent iframe, confirm `allow="tools"` is
      scoped only to that iframe, not inherited broadly.

## 6. Auditability

- [x] Every tool call is logged to `ToolCallLog` (input, output, outcome, timestamp,
...
- [x] Logs never store raw uploaded document bytes or full extracted text — store
